...
export default function EventCard({ title, location, company, id, banner }) {
  return (
    <>
      <Card className={`border-0 hover-shadow mb-5`}>
        <Card.Img variant="top" src={banner} style={{height: '120px', objectFit: 'cover'}} />
        <Card.Body className="px-2">
          ...
          <Card.Text className="d-flex justify-content-between fs-xs text-m">
            <Link to="/vendors/和菓森林">{company.username}</Link>
            <span>
